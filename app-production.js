...
JSON.parse(inputData.mailparser).mail_attachments.forEach(shipment => {
    ...
    if (/^\d+$/.test(Store_ID)) {
        ...
        // 7400
            ...
            async function getOrderData() {
                ...
                orderLookupData = await orderLookup(Order_ID, currentStoreURL, currentAPI_KEY)
                    ...
                    if (item_in_order.final_sku === shipment_SKU_Dashes ){
                        ...
                        async function updateOrder() {
                            ...
                            let response = await fetch(url, {
                                ...
                                // // if error
                                if (!response.ok) {
                                    console.log(`Response Status Not Okay. Item ID: ${itemID}, Shipment Tracking #: ${shipment.tracking_number}, BODY:`, response)
                                // if no error 
                                } if (response.ok) {
                                    console.log(`Item Shipping Updated ${itemID} with tracking number ${shipment.tracking_number}`)
                                }
                        ...
                        console.log(item_in_order.final_sku, "Does Not Match", shipment_SKU_Dashes, shipment);
                    }
    ...
    // for each order with no store id in parentheses the PO number
        ...
        console.error("Store_ID is not valid, it contains a letter: ", Store_ID, shipment);
    }
